# 🐔  基于STM32的智能走地鸡追溯与防丢系统

## 项目名称
**ChickenGuard - 智能散养鸡监护与追溯系统**

## 🎯 项目简介

本项目是一款基于STM32微控制器的智能禽类监护系统，专为走地鸡（散养鸡）品质认证而设计。系统通过RFID射频识别技术为每只散养鸡建立唯一的电子身份档案，结合MPU6050六轴加速度传感器实现实时运动计步统计，能够精确记录散养鸡的日常活动量。同时集成了多传感器联动监控机制，包括光照传感器BH1750、超声波测距模块HCSR04等，构建起全方位的防丢监护网络。系统可验证散养鸡是否达到30天散养标准（每日步数≥5000步），为食品安全追溯提供可靠的数据支撑。OLED显示屏实时展示小鸡编号、累计步数、运动状态等关键信息，配合LED灯光和蜂鸣器提供直观的交互反馈。

### 应用场景

在走地鸡养殖质量认证领域，本系统能够有效解决散养鸡品质验证难题。通过持续监测散养鸡的活动量和行为模式，系统可准确判断其是否达到真正的散养标准，防止圈养鸡冒充散养鸡销售的情况发生。养殖户可通过RFID卡片快速查询每只鸡的运动数据和养殖天数，为消费者提供可追溯的品质保证，从而建立市场信任并提升产品附加值。此外，系统还可扩展应用于其他禽类和畜牧养殖场景，如散养鸭、鹅等家禽的活动量监测与健康管理，以及高价值家禽的防盗防丢和资产保护。通过科技赋能传统养殖业，实现从粗放式管理向精准化、数据化养殖模式的转变。
...
| 模式 | RFID卡片 | 功能描述 |
|-----|---------|---------|
| **小鸡模式** | D4C8F505 | 身份识别、实时计步、防丢监控 |
| **警戒模式** | 012C9902 | 多传感器联动报警（光照/运动/距离） |

### 1.2 核心功能

**🐔 小鸡模式**
- 刷卡识别小鸡身份，蜂鸣器确认
- MPU6050检测运动，X轴变化自动计步
- 步数增加时LED闪烁 + 蜂鸣提示
- OLED显示：模式、编号、步数、状态

**🚨 警戒模式**
- 低光照检测：光照<50lx触发报警
- 快速移动检测：加速度突变>3000触发报警  
- 环境变化检测：超声波距离变化>50cm报警
- LED闪烁 + 持续蜂鸣报警
